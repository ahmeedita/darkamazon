...
import { Gift, DollarSign, ShoppingCart, Euro } from 'lucide-react';
import { useCart } from '@/contexts/CartContext';
import { toast } from 'sonner';
import { AmazonLogo, TargetLogo, EbayLogo, AppleLogo, SteamLogo } from './BrandLogos';
...
export function GiftCard({ product }: GiftCardProps) {
  ...
  const getBrandLogo = (brand: string) => {
    switch (brand.toLowerCase()) {
      ...
      case 'apple': return <AppleLogo className="w-12 h-12 md:w-16 md:h-16 text-gray-400" />;
      case 'steam': return <SteamLogo className="w-12 h-12 md:w-16 md:h-16" />;
      default: return null;
    }
  };
    ...
    <Card className="card-premium p-4 md:p-6 space-y-3 md:space-y-4">
      ...
      <div className="space-y-3 md:space-y-4">
        <div className="text-center py-4 md:py-6 bg-muted/30 rounded-lg">
          ...
          <p className="text-xs md:text-sm text-muted-foreground">Gift Card ({currency})</p>
        </div>
